...
export const settingsSchema = z.object({
  ...
  instrumentTradeCooldownPeriodInHours: z.int().min(0).max(120),
  maximumOrderAgeInMinutes: z.int().min(1).max(1440),
  maximumOpenPositions: z.int().min(0).max(100),
  maximumOpenPositionsAndPendingOrders: z.int().min(0).max(100),
  maximumAlertAgeInSeconds: z.int().min(1).max(86400),
